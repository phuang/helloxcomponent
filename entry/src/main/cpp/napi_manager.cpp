...
Napi::Value NapiManager::GetContext(const Napi::CallbackInfo &info) {
  ...
  case ContextType::APP_LIFECYCLE: {
    ...
    exports.Set("onCreate", Napi::Function::New<NapiManager::NapiOnCreate>(env));
    ...
    exports.Set("onDestroy", Napi::Function::New<NapiManager::NapiOnDestroy>(env));
  }
  ...
  case ContextType::JS_PAGE_LIFECYCLE: {
    ...
    exports.Set("onPageShow", Napi::Function::New<NapiManager::NapiOnPageShow>(env));
    exports.Set("onPageHide", Napi::Function::New<NapiManager::NapiOnPageHide>(env));
  } break;
  default: {
    LOGE("unknown type");
    break;
...
bool NapiManager::Export(Napi::Env env, Napi::Object exports) {
  ...
  auto export_instance = exports.Get(OH_NATIVE_XCOMPONENT_OBJ).As<Napi::Object>();
  auto *native_xcomponent = Napi::ObjectWrap<OH_NativeXComponent>::Unwrap(export_instance);
  ...
  int32_t ret = OH_NativeXComponent_GetXComponentId(native_xcomponent, buf, &size);
...
void NapiManager::OnCreateNative(Napi::Env env) { LOGE("NapiManager::%{public}s()", __func__); }

void NapiManager::OnShowNative() { LOGE("NapiManager::%{public}s()", __func__); }

void NapiManager::OnHideNative() { LOGE("NapiManager::%{public}s()", __func__); }

void NapiManager::OnDestroyNative() { LOGE("NapiManager::%{public}s()", __func__); }

void NapiManager::OnPageShowNative() { LOGE("NapiManager::%{public}s()", __func__); }

void NapiManager::OnPageHideNative() { LOGE("NapiManager::%{public}s()", __func__); }

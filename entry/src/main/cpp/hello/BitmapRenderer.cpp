...
void LoadPicture() {
  ...
  std::call_once(flag, []() {
    ...
    do {
      ...
      Image_ErrorCode retval = OH_ImageSourceNative_CreateFromUri(uri, sizeof(uri), &image_source);
      ...
      LOGE("EEEE size=%{public}dx%{public}d stride=%{public}d format=%{public}d", picture_width_, picture_height_, picture_stride_, format);

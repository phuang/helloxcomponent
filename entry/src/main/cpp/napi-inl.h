...
struct FinalizeData {
  ...
  static inline void Wrapper(node_addon_api_basic_env env,
    ...
    NAPI_FATAL_IF_FAILED(status, "FinalizeData::Wrapper",
...
inline MaybeOrValue<Value> Env::RunScript(String script) const {
  ...
  NAPI_RETURN_OR_THROW_IF_FAILED(*this, status, Napi::Value(*this, result),
                                 Napi::Value);
}
...
inline void BasicEnv::SetInstanceData(T* data) const {
  napi_status status = napi_set_instance_data(
      ...
      nullptr);
  NAPI_FATAL_IF_FAILED(status, "BasicEnv::SetInstanceData",
                       "invalid arguments");
}
...
inline T* BasicEnv::GetInstanceData() const {
  ...
  NAPI_FATAL_IF_FAILED(status, "BasicEnv::GetInstanceData",
                       "invalid arguments");
...
inline const char* BasicEnv::GetModuleFileName() const {
  ...
  NAPI_FATAL_IF_FAILED(status, "BasicEnv::GetModuleFileName",
                       "invalid arguments");
  return result;
}
...
inline void Name::CheckCast(napi_env env, napi_value value) {
  ...
  NAPI_INTERNAL_CHECK(type == napi_string || type == napi_symbol,
                      "Name::CheckCast",
                      "value is not napi_string or napi_symbol, got %d.", type);
}
...
inline void Object::CheckCast(napi_env env, napi_value value) {
  ...
  NAPI_INTERNAL_CHECK(type == napi_object || type == napi_function,
                      "Object::CheckCast",
                      "Expect napi_object or napi_function, but got %d.", type);
}
...
inline Buffer<T> Buffer<T>::NewOrCopy(napi_env env,
  ...
  if (status == details::napi_no_external_buffers_allowed) {
    ...
    // If we can't create an external buffer, we'll just copy the data.
    Buffer<T> ret = Buffer<T>::Copy(env, data, length);
    details::FinalizeData<T, Finalizer, Hint>::WrapperGCWithHint(env, data,
                                                                 finalizeData);
    return ret;
#ifndef NODE_API_NO_EXTERNAL_BUFFERS_ALLOWED
  }
...
inline napi_value InstanceWrap<T>::WrappedMethod(napi_env env,
                                                 napi_callback_info info)
    NAPI_NOEXCEPT {
  return details::WrapCallback([&] {
...
inline void ObjectWrap<T>::FinalizeCallback(node_addon_api_basic_env env,
  ...
  if constexpr (details::HasExtendedFinalizer<T>::value) {
    ...
    NAPI_FATAL_IF_FAILED(status, "ObjectWrap<T>::FinalizeCallback",
                         "node_api_post_finalizer failed");
#else
...
inline napi_value ObjectWrap<T>::WrappedMethod(napi_env env,
                                               napi_callback_info info)
    NAPI_NOEXCEPT {
  return details::WrapCallback([&] {
    const CallbackInfo cbInfo(env, info);
...
TypedThreadSafeFunction<ContextType, DataType, CallJs>::New(
  ...
  napi_status status = napi_create_threadsafe_function(
      env, nullptr, nullptr, String::From(env, resourceName), maxQueueSize,
      initialThreadCount, finalizeData, fini, context, CallJsInternal,
      &tsfn._tsfn);
  if (status != napi_ok) {
    delete finalizeData;
    NAPI_THROW_IF_FAILED(
...
inline void AsyncProgressWorkerBase<DataType>::OnThreadSafeFunctionFinalize(
    Napi::Env env,
    void* /* data */,
    AsyncProgressWorkerBase* context) {
...
inline int64_t MemoryManagement::AdjustExternalMemory(BasicEnv env,
  ...
  NAPI_FATAL_IF_FAILED(status, "MemoryManagement::AdjustExternalMemory",
...
inline uint32_t VersionManagement::GetNapiVersion(BasicEnv env) {
  ...
  NAPI_FATAL_IF_FAILED(status, "VersionManagement::GetNapiVersion",
                       "napi_get_version");
  return result;
}
...
inline const napi_node_version* VersionManagement::GetNodeVersion(
  ...
  NAPI_FATAL_IF_FAILED(status, "VersionManagement::GetNodeVersion",
...
inline void BasicEnv::PostFinalizer(FinalizerType finalizeCallback) const {
  ...
  napi_status status = node_api_post_finalizer(
      _env, details::FinalizeData<T, FinalizerType>::WrapperGCWithoutData,
      static_cast<void*>(nullptr), finalizeData);
  if (status != napi_ok) {
    delete finalizeData;
    NAPI_FATAL_IF_FAILED(status, "BasicEnv::PostFinalizer",
                         "invalid arguments");
  }
}
...
inline void BasicEnv::PostFinalizer(FinalizerType finalizeCallback,
  ...
  napi_status status = node_api_post_finalizer(
      _env, details::FinalizeData<T, FinalizerType>::WrapperGC, data,
      finalizeData);
  if (status != napi_ok) {
    delete finalizeData;
    NAPI_FATAL_IF_FAILED(status, "BasicEnv::PostFinalizer",
...
inline void BasicEnv::PostFinalizer(FinalizerType finalizeCallback,
  ...
  details::FinalizeData<T, FinalizerType, Hint>* finalizeData =
      new details::FinalizeData<T, FinalizerType, Hint>(
          {std::move(finalizeCallback), finalizeHint});
  napi_status status = node_api_post_finalizer(
      _env, details::FinalizeData<T, FinalizerType, Hint>::WrapperGCWithHint,
      data, finalizeData);
  if (status != napi_ok) {
    delete finalizeData;
    NAPI_FATAL_IF_FAILED(status, "BasicEnv::PostFinalizer",

...
#define NAPI_INTERNAL_CHECK_EQ(actual, expected, value_format, location) \
  do {                                                                   \
    ...
    NAPI_INTERNAL_CHECK(actual_value == (expected), location,            \
...
class BasicEnv {
 ...
 public:
  ...
  bool operator==(const BasicEnv& other) const { return _env == other._env; }
  bool operator==(std::nullptr_t /*other*/) const { return _env == nullptr; }
